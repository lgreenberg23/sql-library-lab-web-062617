def select_books_titles_and_years_in_first_series_order_by_year
	...
	ON books.series_id = series.id WHERE series.id = 1 ORDER BY year"
end
...
def select_value_and_count_of_most_prolific_species
	"SELECT species, COUNT(name) AS occurrences
	FROM characters 
	GROUP BY species ORDER BY occurrences DESC LIMIT 1"
end
...
def select_series_title_with_most_human_characters
	...
	HAVING COUNT(characters.series_id) ORDER BY COUNT(characters.series_id) DESC LIMIT 1;"
end
...
def select_character_names_and_number_of_books_they_are_in
	...
	GROUP BY character_books.character_id HAVING count ORDER BY count DESC, characters.name"
end
